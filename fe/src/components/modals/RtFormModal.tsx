...
export default function RtFormModal({ isOpen, onClose, rt, onSave }: RtFormModalProps) {
  const [formData, setFormData] = useState({ name: "" });
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    if (rt) {
      setFormData({ name: rt.name || "" });
    } else {
      setFormData({ name: "" });
    }
  }, [rt, isOpen]);
              ...
              <Dialog.Panel className="w-full max-w-sm transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                <div className="flex justify-between items-center mb-5">
                  <Dialog.Title as="h3" className="text-lg font-bold leading-6 text-slate-800">
                    {rt ? "Edit Data RT" : "Tambah RT Baru"}
                  </Dialog.Title>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Nama RT *</label>
                    <input
                      type="text"
                      name="name"
                      required
