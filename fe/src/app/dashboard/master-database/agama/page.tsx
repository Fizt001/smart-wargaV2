...
export default function MasterAgamaPage() {
  ...
  const fetchAgamas = async () => {
    setIsLoading(true);
    const token = localStorage.getItem("token");
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
      const res = await fetch(`${apiUrl}/api/religions`, {
      ...
      if (res.ok) {
        ...
        toast.error("Gagal mengambil data agama.");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
      const url = editData 
        ? `${apiUrl}/api/religions/${editData.id}` 
        : `${apiUrl}/api/religions`;
  ...
  const handleDelete = async (id: number) => {
    const result = await MySwal.fire({
      ...
      text: "Jika data ini sedang digunakan oleh Warga, penghapusan akan ditolak sistem.",
      icon: 'warning',
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
      const res = await fetch(`${apiUrl}/api/religions/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Master Data Agama</h1>
            <p className="text-slate-500 text-sm mt-1">Kelola daftar agama yang tersedia di dropdown profil warga.</p>
          </div>
          <button 
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          {isLoading ? (
            ...
            <div className="p-16 flex flex-col items-center justify-center text-slate-500">
              ...
              <p>Belum ada data agama yang ditambahkan.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm text-slate-600">
                <thead className="bg-slate-50 text-slate-500 uppercase font-semibold text-xs border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-6 py-4">Nama Agama</th>
                    <th className="px-6 py-4 text-center w-32">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {agamas.map((agama, index) => (
                    <tr key={agama.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4">{index + 1}</td>
                      <td className="px-6 py-4 font-bold text-slate-800">{agama.name}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center space-x-2">
                          <button onClick={() => handleOpenModal(agama)} className="p-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors">
                            <PencilSquareIcon className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDelete(agama.id)} className="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 flex items-center gap-2">
                ...
                {editData ? 'Edit Data Agama' : 'Tambah Data Agama'}
              </h3>
              <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600">&times;</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Nama Agama</label>
                <input
                  type="text"
                  required
                  value={formData.name}
