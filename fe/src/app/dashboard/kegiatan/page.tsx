...
export default function AgendaPage() {
  ...
  const [actFormData, setActFormData] = useState({
    title: "", type: "kegiatan", description: "", activity_date: new Date().toISOString().split('T')[0], budget_proposed: "", is_all_rt: true, target_rts: [] as number[]
  });
  ...
  const fetchData = async () => {
    try {
      ...
      const [res, resWallet, resMaster] = await Promise.all([
        fetch(`${apiUrl}/api/warga/agendas?type=kegiatan`, { headers: { "Authorization": `Bearer ${token}` } }),
  ...
  useEffect(() => {
    ...
    try {
      ...
      setActFormData({ title: "", type: "kegiatan", description: "", activity_date: new Date().toISOString().split('T')[0], budget_proposed: "", is_all_rt: true, target_rts: [] });
      fetchData();
    } catch (error: any) {
      console.error(error);
    }
      ...
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <CalendarDaysIcon className="w-8 h-8 text-blue-600" /> Kegiatan
          </h1>
          <p className="text-slate-500 text-sm mt-1">Daftar kegiatan dan acara di lingkungan Anda.</p>
        </div>
        {user && user.role_id < 8 && (
      ...
      {isAddActOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
            <h3 className="text-lg font-bold text-slate-800 mb-4">Rencanakan Acara Warga</h3>
